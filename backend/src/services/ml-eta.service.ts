...
export class MLEtaService {
  ...
  static predictMultiplier(lat: number, lng: number, distanceMeters: number): number {
    ...
    if (!isWeekend) {
      if (hour >= 7 && hour < 9) multiplier += 0.35;      // AM Peak
      else if (hour >= 9 && hour < 11) multiplier += 0.15; // AM Shoulder
      else if (hour >= 16 && hour < 18) multiplier += 0.45; // PM Peak (Worst)
      else if (hour >= 18 && hour < 20) multiplier += 0.25; // PM Shoulder
    } else {
      if (hour >= 11 && hour < 20) multiplier += 0.15; // Weekend mid-day traffic
    }
    ...
    const isDenseUrban = 
      (lat > 34.02 && lat < 34.06 && lng > -118.27 && lng < -118.23) || // DTLA
